...
static int volvo_tx_hook(CANPacket_t *to_send) {
  ...
  if ( addr == MSG_FSM1_VOLVO_C1 ) {
    ...
    // same factor as 0.04395 PSCM1 message but another offset.
    desired_angle = desired_angle-8192;
    
    
    // WD TIMEOUT in external black panda. Tested fine in C3.
    if (steer_angle_cmd_checks(desired_angle, lka_active, VOLVO_STEERING_LIMITS)) {
      tx = 0;
    }
    
    
    // desired steer angle should be 0 when not controlling steering.
    //if (!controls_allowed && desired_angle != 0) {
    //  tx = 0;
    //}
